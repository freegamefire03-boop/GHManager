...
- Changed: **Settings / Manage Tokens** dialog fully redesigned to a GitHub-style bottom-sheet layout (drag handle, "Tokens" title, THEME segmented control, SAVED token rows with delete icon, NEW TOKEN fields, save-location row with "Change", and Close / Add footer). Cloned from a provided HTML/CSS design.
- Added: **Light mode for the settings sheet.** The panel palette is now theme-aware (adapts to the app's Light/Dark/System theme). Dark: panel `#131A26`, field `#0E1420`, accent `#3B82F6`, text `#E8ECF2`/`#8A94A6`, line `#232D3D`. Light: panel `#FFFFFF`, field `#F0F3F6`, accent `#2563EB`, text `#1F2328`/`#57606A`, line `#D0D7DE`.
- Fixed: **App crashed when opening Settings** — two root causes addressed:
  1. `ModalBottomSheet` renders in a separate window that conflicts with the app's `Theme.AppCompat.DayNight` theme. Reworked the sheet to render via the proven `Dialog` window path (bottom-aligned, rounded-top panel). Replaced custom `BasicTextField` + manual placeholder with standard `OutlinedTextField`; added `verticalScroll` + `weight` for safe layout.
  2. `NullPointerException` in `AndroidParagraphIntrinsics.<init>` during text measurement — release-only, triggered the moment any Text in the dialog was measured. Neither Compose keep-rules nor `-dontoptimize` resolved it, so **R8 minification is temporarily disabled for the release build** (`minifyEnabled false`, `shrinkResources false`). APK is larger but stable. Re-enabling minification with a targeted fix is tracked as a follow-up.
